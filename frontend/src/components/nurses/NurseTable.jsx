...
export default function NurseTable({ nurses = [], onDeactivate, onAssign }) {
              ...
              nurses.map((n) => (
                <tr key={n._id} className="hover:bg-surface-alt transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-text-secondary">{n.email || n.phone || '—'}</p>
                  </td>
                  <td className="px-4 py-3 text-text-secondary">{n.phone || '—'}</td>
                  <td className="px-4 py-3 text-text-secondary">{n.ward || '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${
                      shiftBadge[n.shift] || 'bg-slate-50 text-text-secondary border-slate-200'
                    }`}>
                      {n.shift || '—'}
                    </span>
                  </td>
                    ...
                    <div className="flex items-center gap-1.5">
                      {n.isActive !== false ? (
                        <>
                          ...
                          <span className="text-xs text-status-ok">Active</span>
                        </>
                      ) : (
                        <>
                          <UserX className="w-3.5 h-3.5 text-status-offline" />
                          <span className="text-xs text-status-offline">Inactive</span>
                        </>
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      {n.isActive !== false && (
                        <button
                          onClick={() => onDeactivate && onDeactivate(n)}
